...
class Trilinos(CMakePackage):
    ...
    variant('kokkos-kernels',       default=True,
            description='Compile with Kokkos')
    ...
    conflicts('~kokkos', when='+kokkos-kernels')
    ...
    def cmake_args(self):
        ...
        if '+cuda' in spec:
            options.extend([ '-DCMAKE_CXX_COMPILER=%s' % join_path(spec['nvccwrapper'].prefix.etc, 'nvcc_wrapper') ])
        else:
            options.extend([ '-DCMAKE_CXX_COMPILER=%s' % spec['mpi'].mpicxx ])
        ...
        options.extend([
            ...
            '-DTrilinos_ENABLE_KokkosKernels:BOOL=%s' % (
                'ON' if '+kokkos-kernels' in spec else 'OFF'),
